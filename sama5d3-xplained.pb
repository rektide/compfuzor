...
- hosts: all
  ...
  - shell: chdir="{{SRC}}-at91boot" make ARCH=arm CROSS_COMPILE="{{cc}}" "{{at91boot_target}}"
  - shell: chdir="{{SRC}}-at91boot" make ARCH=arm CROSS_COMPILE="{{cc}}"

  - git: dest="{{SRC}}-uboot" repo="{{uboot_repo}}"
  - get_url: url="{{uboot_patch}}" dest="{{SRCS_DIR}}/sama5d3_xplained-uboot.patch"
  - shell: chdir="{{SRC}}-uboot" patch -p1 < ../sama5d3_xplained-uboot.patch
  - shell: chdir="{{SRC}}-uboot" make ARCH=arm CROSS_COMPILE="{{cc}}" distclean
  - shell: chdir="{{SRC}}-uboot" make ARCH=arm CROSS_COMPILE="{{cc}}" sama5d3_xplained_mmc_config
