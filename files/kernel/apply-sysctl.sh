# apply-sysctl.sh — apply desired sysctl values (from the JSON) to the live kernel.
#
...
while IFS=$'\t' read -r key desired; do
  ...
  if sudo sysctl -w "${key}=${desired}"; then
    ...
    printf 'apply-sysctl.sh: failed to set %s\n' "$key" >&2
    failed=$((failed + 1))
  fi
done <<< "$entries"
