...
# Script to generate files from either command-line arguments or $CRATES_FILE
...
[ ! -f "env.export" ] || source env.export

# Set default template file
[ -n "$OPENCODE_TEMPLATE_FILE" ] && OPENCODE_TEMPLATE_FILE="etc/opencode-rustdocs-mcp.json.template"
[ -n "$CRATES_FILE" ] && CRATES_FILE="etc/crates.txt"

# Check if template file exists
if [ ! -f "$OPENCODE_TEMPLATE_FILE" ]; then
    echo "Error: Template file '$OPENCODE_TEMPLATE_FILE' not found"
    echo "Please create $OPENCODE_TEMPLATE_FILE with CRATE placeholders"
    exit 1
fi

# Read template content
TEMPLATE_CONTENT=$(cat "$OPENCODE_TEMPLATE_FILE")
...
process_crate() {
    ...
    filename="etc/opencode-rustdocs-mcp-${crate}.json"
    ...
    if [ -n "$features" ]; then
        ...
        OUTPUT_CONTENT=$(echo "$TEMP_CONTENT" | jq --arg features "$features" --arg crate "$crate" '
...
if [ $# -eq 0 ]; then
    # No arguments: read from crates.txt
    if [ ! -f "$CRATES_FILE" ]; then
        echo "Error: $CRATES_FILE not found and no arguments provided"
        echo "Usage: $0 <crate> [features]"
        echo "Or create $CRATES_FILE with crate names (one per line)"
        echo "Optional second column for features"
        exit 1
    fi

    echo "Reading from $CRATES_FILE..."
    count=0
    ...
    echo "Done! Generated $count files from $CRATES_FILE."
    ...
    echo "Or run without arguments to read from $CRATES_FILE"
    exit 1
fi
