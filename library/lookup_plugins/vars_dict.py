...
DOCUMENTATION = """
    name: vars_dict
    author: compfuzor
    short_description: Resolve variable names into a key-value dictionary
    description:
      - Works like C(ansible.builtin.vars), but accepts a list of variable names and returns a dictionary of C(variable_name: value).
    options:
      _terms:
        description:
...
EXAMPLES = """
- name: Resolve a list variable containing variable names into a dictionary
  ansible.builtin.set_fact:
    resolved_map: "{{ lookup('vars_dict', my_var_names) }}"

- name: Resolve explicit variable names into a dictionary
  ansible.builtin.set_fact:
    resolved_map: "{{ lookup('vars_dict', ['FOO', 'BAR']) }}"

- name: Resolve with default for missing variable names
  ansible.builtin.set_fact:
    resolved_map: "{{ lookup('vars_dict', ['FOO', 'MISSING'], default='') }}"
"""

RETURN = """
_value:
  description:
    - Dictionary of resolved variable values keyed by variable name.
  type: dict
"""
