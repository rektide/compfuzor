...
| `config_compile` | `(dropins, configs)` | Validate and normalize named drop-in sets and config assembly DAGs; emit ordinary directory/file contributions, a runtime spec, generated bins, and status reporters. | **tolerates `None` → `{}`** for either declaration; malformed records and graph errors raise `AnsibleFilterError`. | [`config_compile.py`](/library/filter_plugins/config_compile.py) |
